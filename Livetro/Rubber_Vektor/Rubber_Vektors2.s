...
wait_Rub
	...
	move.l #screen_FX,bltdpt(a6)
	move.l #$01000000,bltcon0(a6)
	move.w #0,bltdmod(a6)
	move.w #(272*2)<<6+22,bltsize(a6)
	btst #14,dmaconr(a6)
wait_Clear
	btst #14,dmaconr(a6)
	bne.s wait_Clear

	move.l #$257135d1,diwstrt(a6)
	move.l #$003000d8,ddfstrt(a6)
	move.w #$1200,bplcon0(a6)
	move.l #0,bplcon1(a6)
	move.l #0,bpl1mod(a6)
	move.l #$467,color00(a6)
	move.l #screen_FX,bpl1ptH(a6)
...
FX_vbl
	...
	lea $dff000,a6
	movem.l log_FX(pc),a0-a1
	exg a0,a1
	movem.l a0-a1,log_FX
	...
	move.l Coord_Ptr(pc),a1
	movem.w (a1)+,d0-d1
	move.l a1,Coord_Ptr
	cmp.l #Coord_End,a1
	blt.s not_end
	subq.w #1,exit_flag
	move.w #$467,color00(a6)
...
Width2=44				largeur en octets
Heigth2=272				hauteur en pixel
...
.Y_OK
	...
	add.b d4,d4				d4=adr en octets sur X
	add.w d1,d1				d1=d1*2 car table de WORD
	add.w Table_Mulu_Line2(pc,d1.w),d4	d4=d1*Width+d4
	lea 0(a0,d4.w),a1			recherche 1er mot de la droite
	move.w d0,d4				sauvegarde du décalage
	or.w #$0b<<8|MINTERM,d4			source + masque
.find_octant	
	moveq #0,d1
...
.no_SIGNFLAG
	...
	move.l a1,bltcpt(a6)			\ pointeur sur 1er mot droite
	move.l a1,bltdpt(a6)			/
...
MuluCount set 0
	rept Heigth2
	dc.w MuluCount*Width2
MuluCount set MuluCount+1
	endr

DrawLine2_Init
	btst #14,dmaconr(a6)
	bne.s DrawLine2_Init
