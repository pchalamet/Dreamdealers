...
.pp_error
	...
	move.l (sp),d1				essait d'obtenir un Lock
	...
	move.l DOS_Fib+fib_Size,Module_Size-data_base(a5)
	...
	beq.s .allocmem_error

	move.l (sp),d1				ouvre le fichier en lecture
...
.read_error
	...
	CALL Close
.open_error
	move.l Module_Adr(pc),a1		libère la mémoire allouée
	move.l Module_Size(pc),d0
	CALL _ExecBase(pc),FreeMem
.allocmem_error
.dir_error
.examine_error
	move.l Live_Lock(pc),d1			libère le lock sur le module
	CALL _DosBase(pc),UnLock
.lock_error
	clr.l Module_Size-data_base(a5)
	clr.l Module_Adr-data_base(a5)
...
Load_File
	...
	sne Load_Abs-data_base(a5)

	movem.l d0/a0,-(sp)			sauve l'adr et le nom
	subq.l #4,sp				haha... la ruse
...
.load_abs
	...
	move.l 4(sp),File_Adr-data_base(a5)
.skip
	lea 4*3(sp),sp
