...
Display_Gallery
	...
	lea Gallery_Msg,a0			affichage du message d'erreur
	moveq #2,d0
	bsr Display_Text
	bra Gallery_branch
...
Load_Module
	...
	lea -1,a3				function encrypt.. none
...
******* A0=*nom du fichier  d0=adr de chargement -> pas de powerpacker ******
*******                        ou 0 si chargement avec powerpacker     ******
*****************************************************************************
Load_File
	movem.l d0/a0,-(sp)
	bsr Free_File
	...
	tst.l d0				on charge avec powerpacker ?
	beq .load_pp

.load_dos
	movem.l d0/a0,-(sp)			sauve l'adr et le nom
	...
	tst.l DOS_Fib+fib_DirEntryType		c'est un fichier au moins ?
	bge .dir_error

	move.l DOS_Fib+fib_Size,-(sp)		sauve la taille du fichier
	...
	move.l (sp)+,d2				adr de chargement
	addq.l #4,sp				vire le nom du fichier
	CALL Read
	cmp.l d0,d3
	bne.s .read_error
...
.dir_error
.examine_error
	move.l Live_Lock(pc),d1			libère le lock sur le fichier
	CALL UnLock
.lock_error
	addq.l #8,sp
	lea custom_base,a6
...
.load_pp
	moveq #2,d0				col=DECR_POINTER
	move.l #MEMF_CHIP,d1			memtype
	lea File_Adr(pc),a1			&buffer
	lea File_Size(pc),a2			&len
...
Live_Module2		dc.b "st-00:modules-sun/mod.jump and run 2",0
Live_Module3		dc.b "st-00:modules-sun/mod.stand-by-kini",0

Live_Start_Logo1	dc.b "LIVE_1:titlepic/MistraLogo.RAW",0
Live_Start_Logo2	dc.b "LIVE_1:titlepic/Clown.RAW",0
...
SuntheticsMenu
	...
	dc.l %000000
	dcb.l 3,0
	dc.l Sunthetics_Music1
	dc.l Sunthetics_Music2
	dc.l Sunthetics_Music3
	dcb.l 15,0
