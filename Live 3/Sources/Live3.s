...
* ~~~~~~~~~~~~~~~~~~~~~
	incdir "Live2:"
	incdir "Live2:Adverts/"
	incdir "Live2:ClipArts/"
	incdir "Live2:ClipArts/PAK/"
	incdir "Live2:CodeArts/"
	incdir "Live2:Fonts/"
	incdir "Live2:Fonts/RAW/"
	incdir "Live2:Gadgets/"
	incdir "Live2:Gadgets/RAW/"
	incdir "Live2:Gadgets/PAK/"
	incdir "Live2:Layout&TitlePic/RAW"
	incdir "Live2:Layout&TitlePic/PAK"
	incdir "Live2:Menus/"
	incdir "Live2:Messages/"
	incdir "Live2:Music/"
	incdir "Live2:Palettes/"
	incdir "Live2:Sources/"
	incdir "Live2:Zones/"
	incdir "Live2:Articles/"
...
ds_Top		rs.l 1			ptr sur le haut
ds_Bottom	rs.l 1			ptr sur le bas
ds_Mouse	rs.l 1			ptr sur la souris
ds_Border0	rs.l 1			ptr sprite gauche pair
ds_Border1	rs.l 1			ptr sprite gauche impair
ds_Border2	rs.l 1			ptr sprite droit pair
ds_Border3	rs.l 1			ptr sprite droit impair
ds_Colors0	rs.w NB_COLORS		table des couleurs ecrans
ds_Colors1	rs.w 16			table des couleurs sprites
ds_SIZEOF	rs.b 0
...
* DEF_HITBOX active_flag,coordX1,coordY1,LargeX,HautY,buttons,routine
DEF_HITBOX	macro
.gadget\@
	dc.w \1,\2,\3,\2+\4-1,\3+\5-1,\6
	dc.l \7-.gadget\@
	endm
...
* DEF_SHORTCUT active_flag,key_code,routine
DEF_SHORTCUT	macro
.shortcut\@
	dc.b \1,\2
	dc.l \3-.shortcut\@
...
;* DEF_CLIPART coordX,coordY,sizeX,sizeY,ClipArt
;DEF_CLIPART	macro
;.bob\@
;	dc.w \1,\2
;	dc.w (\4*SCREEN_DEPTH<<6)!((\3+15)>>4)
;	dc.w SCREEN_WIDTH-((\3+15)>>3)&$fffe
;	dc.l \5-.bob\@
;	endm
...
* UNE PETITE PRESENTATION POUR LIVE !!!
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Do_Show
	move.l 4.w,a0
	move.w $128(a0),d0
...
.no_big
	move.l sp,save_SP
	lea NewSP+SP_SIZE(pc),sp

	move.l #IT2_Handler,$68.w		installe les handlers de IT
	move.l #IT3_Handler,$6c.w
	move.l #IT6_Handler,$78.w

	bsr Keyboard_init			init le clavier
	IFNE ZIK
...
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	move.w #$e028,intena(a6)		CIA-B / VBL / CIA-A
...
Loop_Init_DZign_Spr
	...
	movem.l ds_Border0(a4),a0-a1		ds_Border0 et ds_Border1
	bsr put_sprite
	or.w #$0080,2(a0)
	move.l (a0),(a1)
	...
	moveq #PART2,d1
	move.w #PART3-PART2+1,d2
	movem.l ds_Border2(a4),a0-a1		ds_Border2 et ds_Border3
	bsr put_sprite
	or.w #$0080,2(a0)
	move.l (a0),(a1)
	lea ds_SIZEOF(a4),a4			DZign suivant
...
IT3_Handler
	...
	move.l IT3_Vbl(pc),a0			recherche l'origine de l'IT
	btst #5,d0
	beq.s IT3_NotFound
IT3_Found
	jsr (a0)
...
* IT6_Handler : gestion replay & clavier
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
IT6_Handler
	SAVE_REGS
	lea data_base(pc),a5
	...
	move.w intreqr(a6),d0
	btst #13,d0				ca viens bien du CIA-B ?
	beq.s IT6_NotFound

	move.b ciabicr,d0
	move.l IT6_Keyboard(pc),a0
	btst #0,d0				ca viens du CIA-B timer A ?
	bne.s IT6_Found	
	move.l IT6_Replay(pc),a0
	btst #1,d0				ca viens du CIA-B timer B ?
	bne.s IT6_Found
	bra.s IT6_NotFound	
IT6_Found
	jsr (a0)
...
Display_Text_Message
	...
	WAIT_BLITTER				attend... c plus prudent!!
loop_display_Text
	moveq #0,d0
...
Init_DZign
	...
	mulu.w #ds_SIZEOF,d0			structure DZign
	lea DZign_List(pc),a4
	lea (a4,d0.l),a4
...
Live_Vbl
	...
	tst.b HP_State-data_base(a5)
	bne.s .no_zik
	move.w mt_percent_to_reach(pc),d0
	cmp.w mt_percent,d0
	beq.s .do_zik
	blt.s .lt
.gt	addq.w #1,mt_percent
	bra.s .do_zik
.lt	subq.w #1,mt_percent
.do_zik	jsr mt_music
.no_zik
	ENDC
...
Fade_Middle_Screen
	...
	bne.s do_fade_out
do_fade_in
	move.l ColorMap_hook-data_base(a5),a0
	addq.l #2,a0
do_fade_out
...
fin_testY
	...
	tst.b Mouse_Flag-data_base(a5)
	beq.s .normal
	neg.w d0				CHITMODE 'MOUSE'
	neg.w d2

.normal
	sub.w d0,MouseX-data_base(a5)		regarde si la souris
	bge.s X_mouse_ok1			est encore dans l'ecran
	clr.w MouseX-data_base(a5)
	bra.s X_mouse_ok2

X_mouse_ok1
	cmp.w #SCREEN_X-1,MouseX-data_base(a5)
	ble.s X_mouse_ok2
	move.w #SCREEN_X-1,MouseX-data_base(a5)

X_mouse_ok2
	sub.w d2,MouseY-data_base(a5)
	bge.s Y_mouse_ok1
	clr.w MouseY-data_base(a5)
	bra.s display_mouse

Y_mouse_ok1
	cmp.w #SCREEN_Y*2-1,MouseY-data_base(a5)
	ble.s display_mouse
	move.w #SCREEN_Y*2-1,MouseY-data_base(a5)
...
Trash_Flag		dc.b 0
			even
IT2_Keyboard		dc.l IT_Keyboard_1	adresses des interuptions
IT3_Vbl			dc.l Live_Vbl		utilisées par les ITx_Handler
IT6_Replay		dc.l 0
IT6_Keyboard		dc.l IT_Keyboard_2

Show_Wait		dc.w 0
mt_percent_to_reach	dc.w 100
DZign_Number		dc.w 0
NbPages			dc.w 1
Barre_Result		dc.w 0
...
save_SP			dc.l 0
NewSP			ds.b SP_SIZE		nouvelle pile
ListPtr			dc.l 0
MessPtr			dc.l 0
Text_Margin		dc.l 0
...
Middle_Default_Colors
	dc.w $234,$A89,$DBA,$C99,$ABB,$EAA,$6A8,$FDC
	dc.w $345,$B9A,$ECB,$DAA,$BCC,$FBB,$7B9,$FED

BackGround_Colors
	dc.w $234,$A89,$DBA,$345,$ABB,$944,$999,$FEE
...
FadeOUT_Table
	dcb.w NB_COLORS,BACKGROUND_COLOR
