...
**
** LOAD_Music "Name","FileName"
**     Chargement d'un module ( FileName ) et stockage sous
**     le nom Name
**
** PLAY_MUSIC "Name"
**     Joue la musique Name
**
...
*************************************************************************************************
	...
	OUTPUT Live:LiveExtensions/MusicManager
...
Extension_Tags
	...
	dc.l 0
Version
	dc.b "$VER: MusicManager v1.0 - (c)1994 Sync/DreamDealers",0
	CNOP 0,4
...
Init_Point
	...
	move.l a5,-(sp)
	bsr mt_SetCIAInt			init les CIAs
	move.l (sp)+,a5
...
Exit_Point
	...
	sf mt_Enable
	move.l ModuleAdr(a5),d0			libère le module si yen a un
	beq.s .no_module
...
.no_module
	...
	move.l a5,-(sp)
	bsr mt_RemCIAInt			vire les CIAs
	move.l (sp)+,a5
...
*************************************************************************************************
*                   INSTRUCTION PERMETTANT DE CHARGER UN MODULE
*
* success=LOAD_MUSIC "FileName"
*
* en entrée: a5=DeathDataBase
*
...
Load_Music
	...
	move.l #ErrorLoadMusic,DeathErrorString(a5)
...
.no_module
	...
	move.l DeathArg0(a5),d1			ouvre le fichier
	...
	move.l d0,ModuleSize(a5)		alloue de la mémoire pour charger
	move.l #MEMF_CHIP,d1			le module
	CALL _ExecBase(a5),AllocMem
	move.l d0,ModuleAdr(a5)
	beq.s no_allocmem

	move.l d7,d1				lit le module
	move.l Mem_Adr(pc),d2
	move.l Mem_Size(pc),d3
	...
	movem.l d7/a5,-(sp)
	move.l ModuleAdr(a5),a0			init le module
	bsr mt_Init
	movem.l (sp)+,d7/a5
	bra.s no_error
	
no_read
	move.l ModuleSize(a5),d0		libère la place mémoire
	move.l ModuleAdr(a5),a1
	CALL _ExecBase(a5),FreeMem
	clr.l ModuleAdr(a5)
no_allocmem
no_seek
no_error
	move.l d7,d1				ferme le fichier module
...
*************************************************************************************************
*                       INSTRUCTION PERMETTANT DE JOUER LE MODULE
*
* success=PLAY_MUSIC
*
* en entrée: a5=DeathDataBase
...
*************************************************************************************************
*                   INSTRUCTION PERMETTANT D'ARRETER DE JOUER LE MODULE
*
* success=STOP_MUSIC
*
* en entrée: a5=DeathDataBase
...
ErrorLoadMusic
	dc.b "Instruction LOAD_MUSIC failed",0
ErrorPlayMusic
	dc.b "Instruction PLAY_MUSIC failed",0
